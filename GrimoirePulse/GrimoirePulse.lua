...
local function toggleButton(parent, key, label, x, y, refresh)
  local b=button(parent,"",x,y,174,function() end)
...
StaticPopupDialogs["GRIMOIREPULSE_SOUND_FOLDER"] = {
  ...
  OnShow = function(self)
    self.editBox:SetText("Interface\\AddOns\\GrimoirePulse\\Sounds\\Custom\\")
...
local function buildOptions()
  ...
  options=CreateFrame("Frame","GrimoirePulseOptions",UIParent,"BackdropTemplate"); options:SetSize(410,425); options:SetPoint("CENTER"); options:SetFrameStrata("DIALOG"); options:SetMovable(true); options:EnableMouse(true); options:RegisterForDrag("LeftButton"); options:SetScript("OnDragStart",options.StartMoving); options:SetScript("OnDragStop",options.StopMovingOrSizing); options:Hide()
  options:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8X8",edgeFile="Interface\\Buttons\\WHITE8X8",edgeSize=2}); options:SetBackdropColor(.025,.012,.05,.97); options:SetBackdropBorderColor(.62,.29,.92,1)
  local header=options:CreateTexture(nil,"ARTWORK"); header:SetPoint("TOPLEFT",2,-2); header:SetPoint("TOPRIGHT",-2,-2); header:SetHeight(67); header:SetColorTexture(.14,.045,.22,.96)
  local cover=options:CreateTexture(nil,"BACKGROUND"); cover:SetTexture(ICON); cover:SetSize(145,145); cover:SetPoint("TOPRIGHT",-12,-9); cover:SetAlpha(.13)
  local rule=options:CreateTexture(nil,"ARTWORK"); rule:SetColorTexture(.66,.38,.9,.72); rule:SetSize(360,1); rule:SetPoint("TOP",0,-70)
  local title=options:CreateFontString(nil,"OVERLAY","GameFontNormalLarge"); title:SetPoint("TOPLEFT",24,-20); title:SetText("|T"..ICON..":28|t  "..L.TITLE); title:SetTextColor(.92,.72,1)
  local subtitle=options:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); subtitle:SetPoint("TOPLEFT",28,-47); subtitle:SetText("✦  "..L.GENERAL.."  ✦"); subtitle:SetTextColor(.72,.45,.94)
  ...
  heading(L.ALERTS,-90)
  local enabled, refreshEnabled=toggleButton(options,"enabled",L.TRACKERS,24,-114)
  local mini, refreshMini=toggleButton(options,"minimap",L.MINIMAP,212,-114)
  heading(L.SOUNDS,-160)
  local lust=button(options,"",24,-184,245,function() cycleSound("lustSound"); options.lustButton:SetText(L.LUST_SOUND..": "..db.lustSound) end)
  local pi=button(options,"",24,-219,245,function() cycleSound("piSound"); options.piButton:SetText(L.PI_SOUND..": "..db.piSound) end)
  options.lustButton, options.piButton = lust, pi
  button(options,L.TEST,280,-184,106,function() play(db.lustSound) end)
  button(options,L.TEST,280,-219,106,function() play(db.piSound) end)
  button(options,L.FOLDER,24,-258,362,function() StaticPopup_Show("GRIMOIREPULSE_SOUND_FOLDER") end)
  heading(L.TRACKERS,-303)
  local move=button(options,"",24,-327,245,function()
  ...
  button(options,L.CLOSE,280,-388,106,function() options:Hide() end)
  local desc=options:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); desc:SetPoint("TOPLEFT",24,-364); desc:SetPoint("TOPRIGHT",-24,-364); desc:SetJustifyH("LEFT"); desc:SetText(L.CUSTOM_HELP)
